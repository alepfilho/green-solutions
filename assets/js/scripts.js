...
icon_1.addEventListener('click', () => {
  if (state_1 === 'menu') {
    ...
    document.querySelector('.menu-mobile-content').classList.add('open');
    ...
    document.querySelector('.menu-mobile-content').classList.remove('open');
    document.querySelector('body').classList.remove('menu-open');
  }
});

/*VERIFICAÇÃO DA RESOLUÇÃO E APLICAÇÃO DOS MENUS*/
...
function getDimensions() {
  ...
  $('.key-logo').css('transform', 'scale(' + largura / 100 / 2 + ')');

  $('.key-logo.g').css('transform', 'scale(' + largura / 100 + ')');
}

let imagem = $('#imagem-color');

$('.inner-servicos .card').hover(function () {
  imagem.css('filter', 'saturate('+ Math.random() * 10 +') grayscale('+ Math.random() * 1 +')');
});
